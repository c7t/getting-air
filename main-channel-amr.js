...
import { check21BalanceOnGPU, allocLevelPool, readPoolIndirection as readPoolIndirectionOn, listActiveBlocks, readCardState , checkRefinementClosureOnGPU , makeCascadePipelines, encodeCascade, cascadeRoundTrip, makeCascadeSeeds, checkSlotQuadrantsOnGPU, makeRenderBindGroup, renderRootIsPool, renderPoolLevels, MAX_RENDER_POOL_LEVELS, makeAMRLayouts, makeCouplingPipelines, makeLevelBindGroups, makeManageBindGroups, makeScheduler, makeRefineRound, readRootFlags, allocRootPool, makeRootPool } from './amr2d-gpu.mjs';
...
async function init() {
  ...
  function resetSim() {
    ...
    device.queue.writeBuffer(cardStateBuf, 0, initCardState());
    // Level 1's PER-BLOCK reset. Under quad allocation the loop below resets
    // it like every other level (U5-4).
    if (!ROOT_MANAGED) {
